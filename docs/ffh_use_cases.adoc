...
xref:ffh_introduction.adoc#resource_descriptor_encoding[], to specify the entry method for a lower
...
xref:#lpi_examples[] provides examples for both a WFI entry method and SBI HSM
...
xref:ffh_introduction.adoc#resource_descriptor_encoding[], for many of its fields.  When using an FFH
...
xref:ffh_lpi_examples.adoc#cppc_examples[] provides examples for both a CSR access and an SBI CPPC
access.
